...
async fn bootstrap(
    ...
    ensure_ready(app, clone_state(&state))
        .await
...
async fn wait_for_existing_server(
    app: &AppHandle,
    state: AppState,
    url: &str,
) -> Result<BootstrapResponse, BootError> {
        ...
        if start.elapsed() > STARTUP_TIMEOUT {
            if let Err(err) = shutdown_process(state.clone()).await {
                emit_log(
                    app,
                    "stderr",
                    format!("cleanup after timeout failed: {err}"),
                );
            }
            return Err(BootError::Message(format!(
                "OpenCode did not answer within {} seconds.",
...
fn acquire_launch_lock() -> Result<fs::File, BootError> {
    let Some(base) = dirs::data_local_dir() else {
        return Err(BootError::Message(
            "Unable to resolve a local data directory.".to_string(),
        ));
    };

    let dir = base.join("opencode-ui");
    fs::create_dir_all(&dir)
        .map_err(|err| BootError::Message(format!("Failed to create app data directory: {err}")))?;
    ...
    file.try_lock_exclusive().map_err(|err| {
        BootError::Message(format!(
            "Another OpenCode UI instance is already managing the server: {err}"
        ))
    })?;
...
fn prepend_path(prefix: &Path) -> String {
    ...
    env::join_paths(parts)
        ...
        .unwrap_or_else(|_| prefix.to_string_lossy().to_string())
}
...
fn prepare_opener_shims() -> Result<PathBuf, BootError> {
    ...
    fs::create_dir_all(&dir).map_err(|err| {
        BootError::Message(format!("Failed to create browser shim directory: {err}"))
    })?;
...
fn create_noop_shim(path: &Path) -> Result<(), BootError> {
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|err| {
            BootError::Message(format!("Failed to prepare opener shim directory: {err}"))
        })?;
    }
    ...
    #[cfg(unix)]
    {
        ...
        let mut perms = fs::metadata(path)
            .map_err(|err| {
                BootError::Message(format!(
                    "Failed to inspect opener shim {}: {err}",
                    path.display()
                ))
            })?
            .permissions();
        ...
        match updater.check().await {
          ...
          .download_and_install(
            move |chunk_length, content_length| {
              let total = downloaded_for_progress.fetch_add(chunk_length, Ordering::Relaxed) + chunk_length;
              let detail = match content_length {
                Some(content_length) if content_length > 0 => {
                  let total = total as u64;
                  let percent = (total as f64 / content_length as f64) * 100.0;
                  format!("{percent:.0}% downloaded ({total}/{content_length} bytes).")
                }
                Some(content_length) => format!("Downloaded {}/{} bytes.", total as u64, content_length),
                None => format!("Downloaded {} bytes.", total as u64),
              };
              emit_status(&progress_app, "downloading", "Updating OpenCode", Some(detail));
            },
            move || {
              emit_status(
                &install_app,
                "installing",
                "Installing update",
                Some("Applying downloaded update.".to_string()),
              );
            },
          )
          .await
        {
          ...
          return;
                ...
                emit_status(
                    ...
                    "restarting",
                    "Restarting OpenCode",
                    Some("Update installed successfully. Restarting now.".to_string()),
                );
                app.restart();
            }
            Ok(None) => {
...
fn main() {
    ...
    let builder = tauri::Builder::default().manage(state).setup(|app| {
        ...
        emit_status(
            ...
            "checking",
            "Checking OpenCode",
            Some(format!("Local server target: {}.", local_url(port))),
        );
